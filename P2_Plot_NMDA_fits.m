...
for tymmn = 1:length(MMN_types)
    ...
    for p = 1:length(participant)
        ...
        if R2_single(p) < 85
            ...
            if strcmp(MMN_types{tymmn},'pMMN')
                subject_array{p,23} = 'needs_NMDA_rerun_pMMN';
            elseif strcmp(MMN_types{tymmn},'dMMN')
                subject_array{p,24} = 'needs_NMDA_rerun_dMMN';
            end
        end
    ...
    title(['NMDA ' modality_data{mode} ' ' MMN_types{tymmn} ' R^2 Individual DCMs']);
    if strcmp(save_figures,'YES')
        saveas(gcf,[outcome_path 'Fits_DCM/histogram_NMDA_' modality_data{mode} '_' MMN_types{tymmn} '.png']);
        close all;
    end
end
end
...
for p = 1:length(participant)
    ...
    title(strcat([modality_data{mode} ' NMDA ' MMN_types{tymmn} ' ' participant{p}]),'Interpreter', 'none')
    legend({'Std(pred)', 'Std(obs)', 'Dev(pred)', 'Dev(obs)'});
    Image = getframe(gcf);    
    if strcmp(save_figures,'YES')
        saveas(gcf,[outcome_path 'Fits_DCM/' modality_data{mode} '_DCM_NMDA_' MMN_types{tymmn} '_' participant{p} '.png']);
        close all;
    end
end
end
